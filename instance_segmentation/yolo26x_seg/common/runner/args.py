...
def parse_common_args(
    ...
    *,
    ...
    include_tracking_toggle: bool = False,
) -> argparse.Namespace:
    ...
    Args:
        ...
        include_tracking_toggle: If ``True``, add ``--no-tracking`` (instance
            segmentation examples with a per-object IoU tracker).
    ...
    # ---- Optional: per-object tracking toggle (instance segmentation) ----
    if include_tracking_toggle:
        parser.add_argument(
            "--no-tracking", dest="tracking", action="store_false", default=True,
            help="Disable the per-object IoU tracker. Instance colors then "
                 "follow raw detection order instead of a stable track id, "
                 "so the same object can change color frame to frame, and "
                 "the flicker-hold (holding a missed detection's last mask "
                 "for a couple of frames) is also disabled.",
        )
